...
from datetime import datetime # Tidak lagi butuh pytz
...
# CoinGecko API untuk data harga dan historis
...
def get_historical_prices(coin_id, days=90):
    ...
    try:
        ...
        if 'prices' in data and data['prices']:
            prices_list = np.array([p[1] for p in data['prices']]) 
            
            ohlc_simulated = []
            if len(prices_list) >= 2:
                start_index = max(0, len(prices_list) - 30) # Ambil 30 harga terakhir untuk OHLC
...
def calculate_ema_np(prices, period):
    ...
    ema = [np.mean(prices_np[:period])] 
    
    for price in prices_np[period:]:
...
def calculate_macd_np(prices, fast_period=12, slow_period=26, signal_period=9):
    ...
    prices_np = np.array(prices)
    if len(prices_np) < max(fast_period, slow_period, signal_period) + 1:
        return "N/A"

    def _calculate_ema_series_np(data, period):
        data_np = np.array(data)
        if len(data_np) < period:
            return np.array([])
        k = 2 / (period + 1)
        ema_series = np.zeros_like(data_np, dtype=float)
        ema_series[period-1] = np.mean(data_np[:period]) 
        for i in range(period, len(data_np)):
            ema_series[i] = (data_np[i] * k) + (ema_series[i-1] * (1 - k))
        return ema_series[period-1:] 

    ema_fast_series = _calculate_ema_series_np(prices_np, fast_period)
    ema_slow_series = _calculate_ema_series_np(prices_np, slow_period)

    min_len = min(len(ema_fast_series), len(ema_slow_series))
    if min_len == 0: return "N/A"

    macd_line_series = ema_fast_series[-min_len:] - ema_slow_series[-min_len:]
    
    if len(macd_line_series) < signal_period:
        return "N/A"

    signal_line_series = _calculate_ema_series_np(macd_line_series, signal_period)
...
def detect_candlestick_pattern(ohlc_data):
    ...
    # --- Pola Kontinuasi / Netral Tambahan ---
    # THREE WHITE SOLDIERS (Bullish Continuation) - Butuh 3 candle
    if len(ohlc_data) >= 3:
        c1, c2, c3 = ohlc_data[-3], ohlc_data[-2], ohlc_data[-1]
        # Semua candle hijau, close lebih tinggi dari open, open c2 > open c1, open c3 > open c2
        if c1['close'] > c1['open'] and c2['close'] > c2['open'] and c3['close'] > c3['open'] and \
           c2['open'] >= c1['open'] and c3['open'] >= c2['open'] and \
           c2['close'] > c1['close'] and c3['close'] > c2['close']: # Semua naik berurutan
            patterns.append({
    ...
    # THREE BLACK CROWS (Bearish Continuation) - Butuh 3 candle
    if len(ohlc_data) >= 3:
        c1, c2, c3 = ohlc_data[-3], ohlc_data[-2], ohlc_data[-1]
        # Semua candle merah, close lebih rendah dari open, open c2 < open c1, open c3 < open c2
        if c1['close'] < c1['open'] and c2['close'] < c2['open'] and c3['close'] < c3['open'] and \
           c2['open'] <= c1['open'] and c3['open'] <= c2['open'] and \
           c2['close'] < c1['close'] and c3['close'] < c2['close']: # Semua turun berurutan
            patterns.append({
    ...
    # Ciri: Open, high, close sama atau sangat dekat, shadow bawah panjang.
    if body_ratio_to_range_c < 0.05 and (open_c - low_c) > (3 * body_c) and (high_c - close_c) < (0.1 * body_c): # Body sangat kecil, long lower shadow, no upper shadow
        patterns.append({
            "name": "DRAGONFLY DOJI",
            "type": "Pembalikan Bullish Kuat",
    ...
    # Ciri: Open, low, close sama atau sangat dekat, shadow atas panjang.
    if body_ratio_to_range_c < 0.05 and (high_c - open_c) > (3 * body_c) and (close_c - low_c) < (0.1 * body_c): # Body sangat kecil, long upper shadow, no lower shadow
        patterns.append({
    ...
    # TWEEZER BOTTOM (Bullish Reversal) - Butuh 2 candle
    if prev_candle and abs(last_candle['low'] - prev_candle['low']) < (0.001 * last_candle['low']): # Low yang sama
        if prev_candle['close'] < prev_candle['open'] and last_candle['close'] > last_candle['open']: # Merah diikuti hijau
            patterns.append({
                "name": "TWEEZER BOTTOM",
                "type": "Pembalikan Bullish Potensial",
    ...
    # TWEEZER TOP (Bearish Reversal) - Butuh 2 candle
    if prev_candle and abs(last_candle['high'] - prev_candle['high']) < (0.001 * last_candle['high']): # High yang sama
        if prev_candle['close'] > prev_candle['open'] and last_candle['close'] < last_candle['open']: # Hijau diikuti merah
            patterns.append({
                "name": "TWEEZER TOP",
                "type": "Pembalikan Bearish Potensial",
    ...
    # Ciri: Body sangat kecil, shadow atas dan bawah sangat panjang dan seimbang.
    ...
    # HIGH WAVE (Indecision)
    # Mirip spinning top dengan ekor sangat panjang.
    if body_ratio_to_range_c < 0.3 and \
    ...
    # SPINNING TOP (Indecision) - Jika belum terdeteksi sebagai Doji atau High Wave
    ...
    if not patterns: # Fallback jika semua deteksi di atas tidak ada yang cocok
        patterns.append({
...
def display_analysis_results(current_price, change_7_day, change_30_day, ema7, ema25, ema99, rsi, macd_status, entry_point, cut_loss_plan, target_profit, market_psychology, trend_24h, ai_prediction, coach_suggestion, is_crypto, is_forex_simulated, long_short_analysis="", candlestick_patterns=None, lot_recommendation=None):
...
def handle_ai_questions(coach_suggestion, market_psychology, cut_loss_plan, asset_name, is_crypto, is_forex_simulated): # is_crypto dan is_forex_simulated sudah ada
...
def analyze_crypto_coin():
    ...
    # Input Modal dan Tipe Trading (Hanya di sini untuk kripto)
    modal = 0.0
    while True:
        modal_input = input("💰 Masukkan modal Anda (contoh: 500000): ").strip() 
        try:
    ...
    while True:
        ...
        ema7 = calculate_ema_np(prices_only_data, 7)
        ema25 = calculate_ema_np(prices_only_data, 25)
        ema99 = calculate_ema_np(prices_only_data, 99)
        rsi = calculate_rsi_np(prices_only_data)
        macd_status = calculate_macd_np(prices_only_data, 12, 26, 9)
        ...
        # --- Logika Deteksi Tren yang Lebih Canggih ---
        # Definisi tren berdasarkan EMA dan pergerakan harga
        if ema25 is not None and ema99 is not None and current_price is not None:
            if current_price > ema25 and ema25 > ema99:
                ...
                entry_point = current_price * 0.99 # Sedikit di bawah harga saat ini untuk entry
                
                # Prediksi Pump/Dump untuk Bullish
                if is_scalper:
                    ai_prediction += "\n🚀 Prediksi: Potensi *pump* cepat dalam waktu dekat jika momentum berlanjut."
                else:
                    ai_prediction += "\n📈 Prediksi: Sangat baik untuk investasi jangka panjang, potensi pertumbuhan stabil."
                ...
                entry_point = current_price * 1.01 # Sedikit di atas harga saat ini untuk short entry

                # Prediksi Pump/Dump untuk Bearish
                if is_scalper:
                    ai_prediction += "\n📉 Prediksi: Potensi *dump* cepat dalam waktu dekat. Waspada!"
                else:
                    ai_prediction += "\n⚠️ Prediksi: Hindari investasi jangka panjang saat ini. Cari titik akumulasi di harga yang lebih rendah."

            elif abs(ema25 - ema99) / ema99 < 0.01: # Jika EMA25 dan EMA99 sangat dekat (kurang dari 1% beda)
                ...
                entry_point = current_price # Entry di harga saat ini jika konsolidasi
                
                # Prediksi Pump/Dump untuk Sideways
                if is_scalper:
                    ai_prediction += "\n🔄 Prediksi: Pergerakan terbatas, cocok untuk *range trading* jika Anda ahli. Potensi *breakout* atau *breakdown*."
                else:
                    ai_prediction += "\n⏳ Prediksi: Fase akumulasi/distribusi. Sabar adalah kunci. Ideal untuk DCA."

            else: # Pergerakan EMA tidak jelas, tapi bukan sideways ketat
                if current_price > ema25:
                    ...
                    entry_point = current_price * 0.995
                else:
                    ...
                    entry_point = current_price * 1.005
        else: # Jika data EMA tidak cukup, fallback ke perubahan 24h
            if perubahan_24h is not None:
                if perubahan_24h > 0:
                    ...
                    entry_point = current_price * 0.998
                    ...
                    entry_point = current_price * 1.002
                else:
        ...
        # Override berdasarkan RSI dan MACD untuk saran yang lebih akurat
        if rsi is not None:
            if rsi > 75: 
                ...
                entry_point = current_price * 1.001 # Untuk short entry di overbought
            elif rsi < 25: 
                ...
                entry_point = current_price * 0.999 # Untuk long entry di oversold
        ...
        if macd_status == "Death Cross":
            if "BUY" in coach_suggestion: 
                 ...
                 entry_point = current_price * 1.001
            ...
            if "SELL" in coach_suggestion: 
                 ...
                 entry_point = current_price * 0.999
...
def analyze_forex_pair():
    ...
    # Input Modal dan Tipe Trading (Hanya di sini untuk Forex)
    modal = 0.0
    while True:
        modal_input = input("💰 Masukkan modal Anda (contoh: 1000000): ").strip() # FIX: Contoh format
        try:
            modal = float(modal_input)
            if modal <= 0:
                raise ValueError
    ...
    historical_simulated_prices = np.array([simulated_price_initial * (1 + random.uniform(-0.01, 0.01)) for _ in range(100)])
    historical_simulated_prices = np.append(historical_simulated_prices, simulated_price_initial)
    ...
    while True:
        ...
        historical_simulated_prices = np.append(historical_simulated_prices, simulated_price)
        ...
        ema7 = calculate_ema_np(historical_simulated_prices, 7)
        ema25 = calculate_ema_np(historical_simulated_prices, 25)
        ema99 = calculate_ema_np(historical_simulated_prices, 99)
        rsi = calculate_rsi_np(historical_simulated_prices)
        macd_status = calculate_macd_np(historical_simulated_prices, 12, 26, 9)
        ...
        # --- Logika Deteksi Tren yang Lebih Canggih untuk Forex ---
        if ema25 is not None and ema99 is not None and simulated_price is not None:
            if simulated_price > ema25 and ema25 > ema99:
                ...
                coach_suggestion = "🟢 SANGAT BUY (Posisi LONG disarankan)"
                long_short_analysis_text = "Sinyal kuat menunjukkan kelanjutan tren naik. Posisi LONG direkomendasikan dengan manajemen risiko ketat. Konfirmasi dari beberapa indikator teknikal."
                entry_point = simulated_price * 0.998
                ...
                coach_suggestion = "🔴 SANGAT SELL (Posisi SHORT disarankan)"
                long_short_analysis_text = "Sinyal kuat menunjukkan kelanjutan tren turun. Posisi SHORT direkomendasikan dengan target profit di level support berikutnya."
                entry_point = simulated_price * 1.002
                ...
                if simulated_price > ema25:
                    ...
                    coach_suggestion = "🟢 BUY (Dengan hati-hati)"
                    long_short_analysis_text = "Tren naik ringan. Posisi LONG bisa dipertimbangkan dengan stop loss ketat."
                    entry_point = simulated_price * 0.999
                else:
                    ...
                    coach_suggestion = "🔴 SELL (Dengan hati-hati)"
                    long_short_analysis_text = "Tren turun ringan. Posisi SHORT bisa dipertimbangkan dengan stop loss ketat."
                    entry_point = simulated_price * 1.001
        else: # Jika data EMA tidak cukup, fallback ke perubahan 24h
            if perubahan_24h is not None:
                if perubahan_24h > 0:
                    pair_trend_24h = "📊 Tren Harian: Bullish Ringan 🐂"
                    ...
                    coach_suggestion = "⏸️ HOLD (Amati)"
                    long_short_analysis_text = "Pergerakan naik jangka pendek. Perlu konfirmasi lebih lanjut."
                    entry_point = simulated_price * 0.9995
                elif perubahan_24h < 0:
                    pair_trend_24h = "📊 Tren Harian: Bearish Ringan 🐻"
                    market_psychology = "Sentimen negatif jangka pendek."
                    ai_prediction = "📉 Pasar menunjukkan pergerakan turun dalam 24 jam terakhir."
                    coach_suggestion = "⏸️ HOLD (Amati)"
                    long_short_analysis_text = "Pergerakan turun jangka pendek. Perlu konfirmasi lebih lanjut."
                    entry_point = simulated_price * 1.0005
                else:
        ...
        risk_percent_for_lot = 0.05 if is_scalper else 0.02 # 5% for scalper, 2% for investor
...
def main_menu():
    ...
    while True: 
        ...
        # Tampilkan sapaan selamat datang
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERROR: Pustaka yang diperlukan tidak ditemukan: {e}. Harap instal dengan: pip install requests numpy")
